...
public class AddConnectorCommandHandler : IRequestHandler<AddConnectorCommand, Response<object>>
{
    ...
    public async Task<Response<object>> Handle(AddConnectorCommand request, CancellationToken cancellationToken)
    {
        ...
        newConnector.ConnectorReportPriorities = request.ConnectorReportPriorities.Select(x => new ConnectorReportPriorities { Name = x.Name, Key = x.Key }).ToList();
        newConnector.ConnectorReportTypes = request.ConnectorReportTypes.Select(x => new ConnectorReportTypes { Name = x.Name, Key = x.Key }).ToList();
        newConnector.Project = await _dbContext.Set<Project>().FindAsync(request.ProjectId, cancellationToken);
